--- STEAMODDED HEADER
--- MOD_NAME: Among Us for All Suits
--- MOD_ID: AmongUsAllSuits
--- MOD_AUTHOR: [Jersen]
--- MOD_DESCRIPTION: Adds the Among Us collab to the other three suits

----------------------------------------------
------------MOD CODE -------------------------

SMODS.DeckSkin {
	key = "collab_AU_clubs",
	suit = "Clubs",
	ranks = {"Jack", "Queen", "King"},
	lc_atlas = "collab_AU_clubs_1",
	hc_atlas = "collab_AU_clubs_2",
	posStyle = "collab",
	loc_txt = {["en-us"] = "Among Us"}
}

SMODS.DeckSkin {
	key = "collab_AU_diamonds",
	suit = "Diamonds",
	ranks = {"Jack", "Queen", "King"},
	lc_atlas = "collab_AU_diamonds_1",
	hc_atlas = "collab_AU_diamonds_2",
	posStyle = "collab",
	loc_txt = {["en-us"] = "Among Us"}
}

SMODS.DeckSkin {
	key = "collab_AU_spades",
	suit = "Spades",
	ranks = {"Jack", "Queen", "King"},
	lc_atlas = "collab_AU_spades_1",
	hc_atlas = "collab_AU_spades_2",
	posStyle = "collab",
	loc_txt = {["en-us"] = "Among Us"}
}

SMODS.Atlas {
	key = "collab_AU_clubs_1",
	path = "collab_AU_clubs_1.png",
	px = 71, 
	py = 95,
}

SMODS.Atlas {
	key = "collab_AU_clubs_2",
	path = "collab_AU_clubs_2.png",
	px = 71, 
	py = 95,
}

SMODS.Atlas {
	key = "collab_AU_diamonds_1",
	path = "collab_AU_diamonds_1.png",
	px = 71, 
	py = 95,
}

SMODS.Atlas {
	key = "collab_AU_diamonds_2",
	path = "collab_AU_diamonds_3.png",
	px = 71, 
	py = 95,
}

SMODS.Atlas {
	key = "collab_AU_spades_1",
	path = "collab_AU_spades_1.png",
	px = 71, 
	py = 95,
}

SMODS.Atlas {
	key = "collab_AU_spades_2",
	path = "collab_AU_spades_2.png",
	px = 71, 
	py = 95,
}

----------------------------------------------
------------MOD CODE END----------------------